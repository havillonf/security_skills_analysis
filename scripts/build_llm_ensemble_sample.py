"""EXP-013 — Amostra para o ensemble de 3 LLMs (E-5/E-6, Decision Log D-026).
...
Amostragem determinística (ORDER BY hash(file_sha)), sem seed externa,
...
DATA_GLOB = str(REPO_ROOT / "data" / "raw" / "gitskills" / "data" / "artifacts" / "*.parquet")
RESULTS_DIR = REPO_ROOT / "results"
CASES_DIR = RESULTS_DIR / "EXP-013_llm_cases"
...
def main() -> None:
    ...
    accepted_out.to_csv(RESULTS_DIR / "EXP-013_llm_sample.csv", index=False)
    ...
    if not rejected_df.empty:
        ...
        rejected_out.to_csv(RESULTS_DIR / "EXP-013_llm_sample_rejected.csv", index=False)
    ...
    (RESULTS_DIR / "EXP-013_llm_sample_summary.json").write_text(
